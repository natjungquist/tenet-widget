...
function customLog (msg, args) {
  const logName = 'cc-widget-logger:'
  if (args) {
...
function changeAgentState (idleCode) {
  ...
  let auxCode
  for (const code of window.ccDesktop.agentStateInfo.latestData.idleCodes) {
...
class myDesktopSDK extends HTMLElement {
  ...
  loadContent () {
    if (!Cookies.get('access_token')) {
      ...
      for (const key of Object.keys(webexService.meetings.meetingCollection.meetings)) {
      ...
      if (Cookies.get('access_token')) {
        ...
        if (selectedRadioButton) {
          ...
          if (selectedValue !== 'manual-sip') {
            ...
            const meeting = webexService.meetings.meetingCollection.meetings[selectedValue]
          ...
          if (sipValid) {
            const interaction = await self.getInteraction()
            customLog('interaction: ', interaction)
            if (interaction) {
              const interactionId = await self.getInteractionId()
              if (interactionId) {
                ...
                const bTResponse = await Desktop.agentContact.blindTransfer({
                  interactionId,
                  data: {
                ...
                const callsResponse = await fetch(`${webexUrl}/telephony/calls`, {
                  method: 'GET',
                  headers
                })
                if (callsResponse.status === 401) {
                  ...
                  const divertResponse = await fetch(`${webexUrl}/telephony/calls/divert`, {
                    method: 'POST',
                    headers,
                    body: JSON.stringify({
                      callId: matchedId,
  ...
  displayMeeting (meeting) {
    try {
      ...
      const name = getMeetingName(meeting)
  ...
  removeMeeting (event) {
    ...
    try {
      const meetingElement = window.shadowRoot.getElementById(event.meetingId)
      ...
      if (!loaded) {
        ...
        Desktop.agentContact.SERVICE.webex.fetchPersonData('me').then(resp => {
          ...
          if (resp.phoneNumbers) {
            for (const n of resp.phoneNumbers) {
